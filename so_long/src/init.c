...
/*   Updated: 2025/08/31 10:15:29 by shfujita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	is_ber_file(const char *path)
{
	size_t	len;
...
int	prepare_map(t_game *g, const char *path)
{
	if (!path)
		return (0);
	...
	if (!load_map(path, &g->map))
		return (0);
	if (!is_rect_map(&g->map) || !is_correct_charset(&g->map)
		|| !is_surrounded_walls(&g->map) || !count_player_collect_exit(&g->map)
		|| !check_path(&g->map))
	{
		free_map(&g->map);
		return (0);
