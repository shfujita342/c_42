...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/02 19:43:38 by shfujita          #+#    #+#             */
/*   Updated: 2025/10/02 20:21:01 by shfujita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			else
// 			{
// 				infd = execute_heredoc(cmd->infile[count], shell,
// 						in_quotes[count]);
// 			}
// 			count++;
// 		}
...
static int	handle_infile(int i, t_shell *sh)
{
	int			count;
	int			infd;
	char		*infile;
	...
	{
		if (cmd->is_heredoc[count])
			infd = execute_heredoc(cmd->infile[count], sh,
					cmd->in_quotes[count]);
		else
		{
			infile = cmd->infile[count];
...
static int	handle_outfile(t_command *cmd)
{
	int		outfd;
	int		count;
	int		flags;
...
static int	set_infd(int i, int **pipes, t_shell *sh)
{
	t_command	*cmd;
	int			infd;
	int			count;
	...
	cmd = sh->current_pipeline->commands;
	count = 0;
	if (cmd->infile_count > 0)
		infd = handle_infile(i, cmd);
	else if (i > 0)
		infd = pipes[i - 1][0];
	if (infd >= 0)
	{
...
void	set_command_fds(t_shell *sh, int i, int **pipes)
{
	...
	if (set_infd(i, pipes, sh) < 0 || set_outfd(i, n_pipes, pipes, cmd) < 0)
	{
